...
static GameRestorableSessionData sessionData;
...
void gameSession_saveGameState() {
    ...
    sessionData.gameMode = gameSession.gameMode;
    ...
    PrefSetAppPreferences(APP_CREATOR_ID, 1, 1, &sessionData, sizeof(GameRestorableSessionData), true);
}
...
*/
    UInt16 prefsSize = 0;
    if (PrefGetAppPreferences(APP_CREATOR_ID, 1, &sessionData, &prefsSize, true) == noPreferenceFound)
        return NULL;
    if (!database_levelPackExists(sessionData.dbId) || (database_levelCount(sessionData.dbId) <= sessionData.level))
        return NULL;
    return &sessionData;
...
static void gameSession_progressGameLogicPenInput() {
//debug(1, "XXX", "progressGameLogicPenInput paused=%d wasUpdated=%d touching=%d didMove=%d", gameSession.paused, gameSession.lastPenInput.wasUpdatedFlag, gameSession.lastPenInput.touching, gameSession.lastPenInput.didMove);
...
//debug(1, "XXX", "progressGameLogicPenInput handleButtonTap");
            return;
        }
        if (gameSession_handleLevelEditor()) {
//debug(1, "XXX", "progressGameLogicPenInput handleLevelEditor");
            return;
        }

        if (gameSession.lastPenInput.touching && gameSession.lastPenInput.didMove) {
            gameSession.isAiming = true;
//debug(1, "XXX", "progressGameLogicPenInput set isAiming true");
            gameSession.userScroll = false;
        } else if (!gameSession.lastPenInput.touching) {
//debug(1, "XXX", "progressGameLogicPenInput set isAiming false");
            gameSession.isAiming = false;
