...
static LevelPacked *database_getPackedLevel(uint8_t *p) {
  LevelPacked *packed = NULL;
  UInt16 count, index, coord;
  UInt8 itemType;
  int i;

  if (p) {
    i = get2b(&count, p, 0);
    packed = MemPtrNew(sizeof(LevelPacked) + (count - 1) * sizeof(LevelItem));
    packed->levelItemCount = count;

    for (index = 0; index < packed->levelItemCount; index++) {
      i += get1(&itemType, p, i);
      i++;
      packed->levelItems[index].itemType = itemType;
      i += get2b(&coord, p, i);
      packed->levelItems[index].position.x = coord;
      i += get2b(&coord, p, i);
      packed->levelItems[index].position.y = coord;
      i += get2b(&coord, p, i);
      packed->levelItems[index].positionB.x = coord;
      i += get2b(&coord, p, i);
      packed->levelItems[index].positionB.y = coord;
    }
  }
...
static void database_ungetPackedLevel(LevelPacked *packed) {
  if (packed) MemPtrFree(packed);
}
...
void database_readPackedLevel(Level *level, UInt16 index, LocalID dbId) {
    ...
    packed = database_getPackedLevel(p);
    if (!packed) {
    ...
    database_ungetPackedLevel(packed);
