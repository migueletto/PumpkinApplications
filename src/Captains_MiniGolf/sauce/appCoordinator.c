...
Int32 timeout = appCoordinator_eventDelayTime(currentActivity);
EvtGetEvent(&event, timeout);
//if (event.eType != nilEvent) {
//debug(1, "XXX", "EventDispatcher timeout %d %s %d,%d", timeout, EvtGetEventName(event.eType), event.screenX, event.screenY);
//}
