...
void m6809_optable(UInt8 op, m6809_Regs *m6809) {
  switch (op) {
...
void m6809_reset(m6809_Regs *m6809) {
  ...
  m6809->hardware->eventcount = 0;
  m6809->hardware->totalcycles = 0;
  m6809->hardware->firq_request = 0;
...
Int32 m6809_execute(m6809_Regs *m6809, Int32 cycles) {
  ...
  if (m6809->int_state & (M6809_CWAI | M6809_SYNC)) {
    ...
    do {
      ...
      m6809_ICount -= m6809->hardware->cycles[ireg];
      m6809->hardware->totalcycles += m6809->hardware->cycles[ireg];
      m6809->hardware->eventcount += m6809->hardware->cycles[ireg];

      if (m6809->hardware->firq_request) {
        m6809->hardware->firq_request = 0;
        IO_cart(m6809->hardware, 0);
        m6809_set_irq_line(m6809, M6809_FIRQ_LINE, ASSERT_LINE);
        break;

      } else {
        if (m6809->hardware->totalcycles >= m6809->hardware->vsync) {
          m6809->hardware->totalcycles = 0;
          IO_vsync(m6809->hardware, 0);
          if (m6809->hardware->vsync_irq)
...
void fetch_effective_address(m6809_Regs *m6809) {
  UInt8 postbyte = ROP_ARG(PC);
  PC++;
