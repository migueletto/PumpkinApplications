...
void m6502_optable(m6502_Regs *m6502);
void m6502_adrmode(m6502_Regs *m6502);
void nmi6502(m6502_Regs *m6502);
void irq6502(m6502_Regs *m6502);

void m6502_init(m6502_Regs *m6502) {
}

void m6502_reset(m6502_Regs *m6502) {
  m6502->hardware->eventcount = 0;
  m6502->hardware->totalcycles = 0;
  m6502->hardware->irq_request = 0;
  m6502->hardware->nmi_request = 0;
...
Int32 m6502_execute(m6502_Regs *m6502, Int32 cycles) {
  while (cycles > 0) {
    if (m6502->hardware->nmi_request) {
      m6502_set_irq_line(m6502, M6502_NMI_LINE, ASSERT_LINE);
      m6502->hardware->nmi_request = 0;
    } else if (m6502->hardware->irq_request) {
    ...
    m6502_optable(m6502);
    m6502->hardware->totalcycles += m6502->hardware->cycles[m6502->opcode];
    m6502->hardware->eventcount += m6502->hardware->cycles[m6502->opcode];
    m6502->clockticks6502 += m6502->hardware->cycles[m6502->opcode];
    cycles -= m6502->clockticks6502;
    m6502->clockticks6502 = 0;

    if (m6502->hardware->useevents &&
        m6502->hardware->totalcycles >=
          m6502->hardware->ecycle[m6502->hardware->event]) {
      if (m6502->hardware->event == m6502->hardware->nevents) {
        m6502->hardware->totalcycles = 0;
        m6502->hardware->event = 0;
      }
      IO_event(m6502->hardware, m6502->hardware->event,
        m6502->hardware->earg[m6502->hardware->event]);
      m6502->hardware->event++;
    }

    if (m6502->hardware->vsync &&
        m6502->hardware->totalcycles >= m6502->hardware->vsync) {
      m6502->hardware->totalcycles = 0;
      IO_vsync(m6502->hardware, 0);
      switch (m6502->hardware->vsync_irq) {
        case 1: m6502->hardware->irq_request = 1; break;
        case 2: m6502->hardware->nmi_request = 1;
      }
    }
  }
...
// Implied
void implied6502(m6502_Regs *m6502) {
}

// #Immediate
void immediate6502(m6502_Regs *m6502) {
  SAVEPC = PC++;
}

// ABS
void abs6502(m6502_Regs *m6502) {
  SAVEPC = RM16(PC);
  PC++;
  PC++;
}
...
// Branch
void relative6502(m6502_Regs *m6502) {
  SAVEPC = RM(PC);
  PC++;
...
// (ABS)
void indirect6502(m6502_Regs *m6502) {
  HELP = RM16(PC);
  SAVEPC = RM16(HELP);
  PC++;
  PC++;
...
// ABS,X
void absx6502(m6502_Regs *m6502) {
  SAVEPC = RM16(PC);
  PC++;
  PC++;
  if (m6502->hardware->cycles[m6502->opcode] == 4)
...
// ABS,Y
void absy6502(m6502_Regs *m6502)
{
  SAVEPC = RM16(PC);
  PC++;
  PC++;
...
// (ZP,X)
void indx6502(m6502_Regs *m6502)
{
  VALUE = RM(PC) + X;
  PC++;
  SAVEPC = RM16(VALUE);
...
// (ZP),Y
void indy6502(m6502_Regs *m6502)
{
  VALUE = RM(PC);
  PC++;
  SAVEPC = RM16(VALUE);
  if (m6502->hardware->cycles[m6502->opcode] == 5)
    if ((SAVEPC >> 8) != ((SAVEPC + Y) >> 8))
      m6502->clockticks6502++;
  SAVEPC += Y;
}
...
// (ABS,X)
void indabsx6502(m6502_Regs *m6502)
{
  HELP = RM16(PC) + X;
  SAVEPC = RM16(HELP);
}
...
// (ZP)
void indzp6502(m6502_Regs *m6502)
{
  VALUE = RM(PC);
  PC++;
  SAVEPC = RM16(VALUE);
...
void asla6502(m6502_Regs *m6502)
{
  P = (P & 0xfe) | ((A >> 7) & 0x01);
  A = A << 1;
  if (A) P &= 0xfd; else P |= 0x02;
...
void clc6502(m6502_Regs *m6502)
{
  P &= 0xfe;
}

void cld6502(m6502_Regs *m6502)
{
  P &= 0xf7;
}

void cli6502(m6502_Regs *m6502)
{
  P &= 0xfb;
}

void clv6502(m6502_Regs *m6502)
{
  P &= 0xbf;
}

void cmp6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  VALUE = RM(SAVEPC);
...
void jmp6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  PC = SAVEPC;
}

void jsr6502(m6502_Regs *m6502)
{
  PC++;
  WM(0x0100+S--, PC >> 8);
  WM(0x0100+S--, PC & 0xff);
...
void lsra6502(m6502_Regs *m6502)
{
  P = (P & 0xfe) | (A & 0x01);
  A = A >>1;
  if (A) P &= 0xfd; else P |= 0x02;
...
void nop6502(m6502_Regs *m6502)
{
}

void ora6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  A |= RM(SAVEPC);
  if (A) P &= 0xfd; else P |= 0x02;
...
void pha6502(m6502_Regs *m6502)
{
  WM(0x100+S--, A);
}

void php6502(m6502_Regs *m6502)
{
  WM(0x100+S--, P);
}

void pla6502(m6502_Regs *m6502)
{
  A = RM(++S+0x100);
  if (A) P &= 0xfd; else P |= 0x02;
  if (A & 0x80) P |= 0x80; else P &= 0x7f;
...
void plp6502(m6502_Regs *m6502)
{
  P = RM(++S+0x100) | 0x20;
}

void rol6502(m6502_Regs *m6502)
{
  m6502->saveflags = (P & 0x01);
  m6502_adrmode(m6502);
  VALUE = RM(SAVEPC);
...
void rola6502(m6502_Regs *m6502)
{
  m6502->saveflags = P & 0x01;
  P = (P & 0xfe) | ((A >> 7) & 0x01);
  A = A << 1;
...
void rora6502(m6502_Regs *m6502)
{
  m6502->saveflags = P & 0x01;
  P = (P & 0xfe) | (A & 0x01);
  A = A >> 1;
...
void sec6502(m6502_Regs *m6502)
{
  P |= 0x01;
}

void sed6502(m6502_Regs *m6502)
{
  P |= 0x08;
}

void sei6502(m6502_Regs *m6502)
{
  P |= 0x04;
}

void sta6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  WM(SAVEPC, A);
}

void stx6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  WM(SAVEPC, X);
}

void sty6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  WM(SAVEPC, Y);
}

void tax6502(m6502_Regs *m6502)
{
  X = A;
  if (X) P &= 0xfd; else P |= 0x02;
  if (X & 0x80) P |= 0x80; else P &= 0x7f;
...
void txs6502(m6502_Regs *m6502)
{
  S = X;
}

void tya6502(m6502_Regs *m6502)
{
  A = Y;
  if (A) P &= 0xfd; else P |= 0x02;
  if (A & 0x80) P |= 0x80; else P &= 0x7f;
...
void phx6502(m6502_Regs *m6502)
{
  WM(0x100+S--, X);
}

void plx6502(m6502_Regs *m6502)
{
  X = RM(++S+0x100);
  if (X) P &= 0xfd; else P |= 0x02;
  if (X & 0x80) P |= 0x80; else P &= 0x7f;
...
void phy6502(m6502_Regs *m6502)
{
  WM(0x100+S--, Y);
}

void ply6502(m6502_Regs *m6502)
{
  Y = RM(++S+0x100);
  if (Y) P &= 0xfd; else P |= 0x02;
  if (Y & 0x80) P |= 0x80; else P &= 0x7f;
...
void stz6502(m6502_Regs *m6502)
{
  m6502_adrmode(m6502);
  WM(SAVEPC, 0);
}

void tsb6502(m6502_Regs *m6502)
{
  UInt8 tmp;
  m6502_adrmode(m6502);
  tmp = RM(SAVEPC) | A;
