...
void z80_reset(z80_Regs *z80) {
  ...
  z80->hardware->eventcount = 0;
  z80->hardware->totalcycles = 0;
...
static void take_interrupt(z80_Regs *z80) {
  UInt16 vector;
  ...
  if (IFF1) {
    ...
    switch (IM) {
      ...
      case 1:
        ...
        break;
      case 2:
        vector = (vector & 0x00FF) | (IR & 0xFF00);
        PUSH(PC);
        PC = GetWORD(vector);
        //z80->extra_cycles += z80->cycles[0xCD];
    }
  }
}
...
static void cb_prefix(z80_Regs *z80, FASTREG adr) {
...
static FASTREG dfd_prefix(z80_Regs *z80, FASTREG IXY) {
...
Int32 z80_execute(z80_Regs *z80, Int32 cycles) {
  FASTWORK temp, acu, sum, cbits, op;
  UInt8 ireg;
  ...
  case 0xDB:      /* IN A,(nn) */
    ...
    //Sethreg(AF, Input((AF & 0xFF00) | GetBYTE(PC))); ++PC;

    temp = GetBYTE(PC);
    if (temp == 1 && !CheckInput(temp)) {
      IO_halt(z80->hardware, 1);
      z80_ICount = 0;
      PC--;
      break;
    }

    Sethreg(AF, Input((AF & 0xFF00) | temp)); ++PC;
    break;
  case 0xDC:      /* CALL C,nnnn */
    CALLC(TSTFLAG(C));
    break;
  case 0xDD:      /* DD prefix */
    IX = dfd_prefix(z80, IX);
    break;
  case 0xDE:      /* SBC A,nn */
    temp = GetBYTE(PC);
    acu = hreg(AF);
  ...
  case 0xFF:      /* RST 38H */
    ...
    z80_ICount -= z80->hardware->cycles[ireg];
    z80->hardware->eventcount += z80->hardware->cycles[ireg];

    if (z80->hardware->vsync) {
      z80->hardware->totalcycles += z80->hardware->cycles[ireg];
      if (z80->hardware->totalcycles >= z80->hardware->vsync) {
        z80->hardware->totalcycles = 0;
        IO_vsync(z80->hardware, 0);
        if (z80->hardware->vsync_irq)
          z80_set_irq_line(z80, z80->hardware->vsync_irq - 1, 1);
      }
    }
