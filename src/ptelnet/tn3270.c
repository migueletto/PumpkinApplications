...
static UInt8 ebcdic2ascii(UInt8);
static UInt8 ascii2ebcdic(UInt8);
static UInt8 FromBuffer(Int16);
static Int16 ToBuffer(UInt8);
static void ToAttr(Int16, UInt8 c);
static void MakeBA_12A(UInt16, UInt8 *b);
static Int16 FirstField(void);
static Int16 NextField(Int16 ba);
static Int16 CurrentField(Int16);
...
void EmitTN3270(Char *buf, Int16 n, Int16 done) {
  ...
  debug(DEBUG_TRACE, "3270", "received %d bytes (state %d)", n, tn3270_s);
  debug_bytes(DEBUG_TRACE, "3270", (UInt8 *)buf, n);
  ...
  for (i = 0; i < n; i++) {
    ...
    switch (tn3270_s) {
      case 1:	// CMD
        switch (c) {
          ...
          case SNA_CMD_EW:
            ...
            cursorBA = -1;
            DmSet(Buffer, 0, BufferSize*2, 0);
            debug(DEBUG_TRACE, "3270", "< CMD Erase %02X", c);
            // fall-through
          case CMD_W:
          case SNA_CMD_W:
            CMD = CMD_W;
            tn3270_s = 2;
            debug(DEBUG_TRACE, "3270", "< CMD Write %02X", c);
            break;
          case CMD_RM:
          case SNA_CMD_RM:
            if (AID == AID_CLEAR || AID == AID_PA1 || AID == AID_PA2) {
              ...
              tn3270_s = 1;
              debug(DEBUG_TRACE, "3270", "< CMD Short Read %02X", AID);
              break;
            }
            // fall-through
          case CMD_RMA:
          case SNA_CMD_RMA:
            debug(DEBUG_TRACE, "3270", "< CMD Read %02X", c);
            CMD = c;
            debug(DEBUG_TRACE, "3270", "send AID %02X", AID);
            SendTelnet(&AID, 1, false);
            MakeBA_12A(BA, codBA);
            debug(DEBUG_TRACE, "3270", "send BA %d", BA);
            ...
            for (k = 0; k < BufferSize; k++) {
              ...
              if (!FA_PROTECTED(FA) && FA_MDT(FA)) {     // se o campo foi modificado
                if ((next = NextField(k)) != -1) {
                  ...
                  debug(DEBUG_TRACE, "3270", "field %d modified len %d FA %02X", k, len, FA);
                  c = ORD_SBA;
                  SendTelnet(&c, 1, false);
                  // inicio do campo, apos o Field Attribute
                  tmpBA = (k + 1) % BufferSize;
                  MakeBA_12A(tmpBA, codBA);
                  debug(DEBUG_TRACE, "3270", "send SBA %d", tmpBA);
                  SendTelnet(codBA, 2, false);
                  for (j = 0; j < len; j++) {
                    c = FromBuffer(tmpBA);
                    tmpBA = (tmpBA + 1) % BufferSize;
                    if (c) { // RM/RMA suprimem nulos
                      debug(DEBUG_TRACE, "3270", "field %d pos %d: %02X ('%c')", k, j, c, ebcdic2ascii(c));
                      SendTelnet(&c, 1, false);
                    }
                  }
          ...
          case CMD_RB:
          case SNA_CMD_RB:
            debug(DEBUG_TRACE, "3270", "< CMD Read Buffer %02X", c);
            CMD = c;
            AID = AID_NO;
            SendTelnet(&AID, 1, false);
            MakeBA_12A(BA, codBA);
          ...
          case CMD_EAU:
          case SNA_CMD_EAU:
            debug(DEBUG_TRACE, "3270", "< CMD Erase AU %02X", c);
            CMD = c;
            for (k = 0; k < BufferSize; k++) {
              if (!Attr[k]) continue;
              FA = Attr[k] & 0x7F;
          ...
          case SNA_CMD_WSF:
            tn3270_s = 12;
          default:
            debug(DEBUG_TRACE, "3270", "unknown CMD %02X", c);
            break;
        }
        break;
      ...
      case 2:	// WCC
        WCC = c;
        debug(DEBUG_TRACE, "3270", "< WCC %02X", WCC);
        if ((WCC & 0x04)) {
          // sound alarm bit
        }
        if ((WCC & 0x02)) {
      ...
      case 3:	// Orders and data
        switch (c) {
          case ORD_SF:
            debug(DEBUG_TRACE, "3270", "< ORD SF %02X", c);
            tn3270_s = 4;
            break;
          case ORD_SBA:
            debug(DEBUG_TRACE, "3270", "< ORD SBA %02X", c);
            ORD = c;
            tn3270_s = 5;
            break;
          case ORD_SFE:		// ignored
            debug(DEBUG_TRACE, "3270", "< ORD SFE %02X", c);
            tn3270_s = 8;
            break;
          case ORD_SA:
            debug(DEBUG_TRACE, "3270", "< ORD SA %02X", c);
            ORD = c;
            npairs = 1;
            tn3270_s = 9;
            break;
          case ORD_MF:		// ignored
            debug(DEBUG_TRACE, "3270", "< ORD MF %02X", c);
            tn3270_s = 8;
            break;
          case ORD_IC:
            debug(DEBUG_TRACE, "3270", "< ORD IC %02X", c);
            cursorBA = BA;
            tn3270_s = 3;
            break;
          case ORD_PT:		// ignored
            // advances the current buffer address to the address of the first character position of the next unprotected field
            debug(DEBUG_TRACE, "3270", "< ORD PT %02X", c);
            tn3270_s = 3;
            break;
          case ORD_RA:
            ...
            debug(DEBUG_TRACE, "3270", "< ORD RA %02X", c);
            ORD = c;
            tn3270_s = 5;
            break;
          case ORD_EUA:
            debug(DEBUG_TRACE, "3270", "< ORD EUA %02X", c);
            ORD = c;
            tn3270_s = 5;
            break;
          case ORD_GE:
            debug(DEBUG_TRACE, "3270", "< ORD GE %02X", c);
            tn3270_s = 7;
            break;
          default:
            a = ebcdic2ascii(c);
            debug(DEBUG_TRACE, "3270", "< data %02X '%c' (%02X %d)", c, a, FA, FA_INTENSIFIED(FA) ? 1 : 0);
            if (CMD == CMD_W) {
              ToAttr(BA, 0);
              ToBuffer(c);
              EmitChar3270(a, FA);
      ...
      case 4:	// Start Field: field attribute
        FA = c;
        debug(DEBUG_TRACE, "3270", "Start Field @ %d FA %02X (%s)", BA, FA, FA_PROTECTED(FA) ? "protected" : "unprotected");
        ToAttr(BA, FA | 0x80);
        ToBuffer(0);
        EmitChar3270(' ', FA);
        tn3270_s = 3;
      ...
      case 5:	// Set Buffer Address: byte 1
        debug(DEBUG_TRACE, "3270", "< SBA1 %02X", c);
        switch (ba_mode = BA_MODE(c)) {
          ...
          case BA_RESERVED:
            tmpBA = 0;
            debug(DEBUG_TRACE, "3270", "< BA_RESERVED");
        }
        tn3270_s = 6;
        break;

      case 6:	// Set Buffer Address: byte 2
        debug(DEBUG_TRACE, "3270", "< SBA2 %02X", c);
        switch (ba_mode) {
          case BA_14:  
            tmpBA = tmpBA | (UInt16)c;
            debug(DEBUG_TRACE, "3270", "< BA_14 %d", tmpBA);
            break;
          case BA_12A: 
          case BA_12B:
            tmpBA = tmpBA | (UInt16)(c & 0x3f);
            debug(DEBUG_TRACE, "3270", "< BA_12 %d", tmpBA);
        }
        switch (ORD) {
          case ORD_SBA:
            if (tmpBA < BufferSize) {
              ...
              debug(DEBUG_TRACE, "3270", "< ORD_BA Err");
            }
            tn3270_s = 3;
            break;
          case ORD_RA:
            debug(DEBUG_TRACE, "3270", "< ORD_RA");
            tn3270_s = 11;
            break;
          case ORD_EUA:
            debug(DEBUG_TRACE, "3270", "< ORD_EUA");
            tn3270_s = 3;
        }
        break;

      case 7:	// Graphic Escape
        debug(DEBUG_TRACE, "3270", "< GE %02X", c);
        tn3270_s = 3;
        break;

      case 8:	// Start Field Extended: number of attr. type-value pairs
        npairs = c;
        debug(DEBUG_TRACE, "3270", "< SF ext npairs %d", npairs);
        tn3270_s = npairs ? 9 : 3;
        break;

      case 9:	// Type-Value pair: type
        debug(DEBUG_TRACE, "3270", "< type %02X", c);
        sa_type = c;
        tn3270_s = 10;
        break;

      case 10:	// Type-Value pair: value
        debug(DEBUG_TRACE, "3270", "< value %02X", c);
        sa_value = c;
        npairs--;
        tn3270_s = npairs ? 9 : 3;
        ...
        switch (ORD) {
          case ORD_SA:
            debug(DEBUG_TRACE, "3270", "set attribute type %02X value %02X", sa_type, sa_value);
      ...
      case 11:	// Order Repeat to Address: character
        debug(DEBUG_TRACE, "3270", "< order rep %02X, %d to %d", c, BA, tmpBA);
        if (c == ORD_GE) {
          tn3270_s = 7;
        } else {
          FA = 0;
      ...
      case 12:	// Write Structured Field: length, byte 1
        debug(DEBUG_TRACE, "3270", "< Write SF len1 %02X", c);
        length = ((UInt16)c) << 8;
        tn3270_s = 13;
        break;

      case 13:	// Write Structured Field: length, byte 2
        debug(DEBUG_TRACE, "3270", "< Write SF len2 %02X", c);
        length |= (UInt16)c;
        debug(DEBUG_TRACE, "3270", "< Write SF %04X", length);
      ...
      case 14:	// Write Structured Field: field
        debug(DEBUG_TRACE, "3270", "< Write SF byte %d %02X", isfcmd, c);
        if (isfcmd < 3) {
          SFCMD[isfcmd++] = c;
        }
        length--;
        if (!length) {
          ...
          if (SFCMD[0] == 0x01 && SFCMD[1] == 0xff && SFCMD[2] == 0x02) {
            debug(DEBUG_TRACE, "3270", "< Read Partition");
            debug(DEBUG_TRACE, "3270", "Send Partition (%d bytes)", replySize);
...
Err SendTN3270(UInt16 c) {
  ...
  switch (c) {
    ...
    case 10:
      debug(DEBUG_TRACE, "3270", "ENTER key BA %d", BA);
      AID = AID_ENTER;
    ...
    case 8:		// Backspace
      ...
      break;
    default:
      debug(DEBUG_TRACE, "3270", "key %02X BA %d", c, BA);
      if ((i = CurrentField(BA)) != -1) {
        FA = Attr[i] & 0x7F;
        debug(DEBUG_TRACE, "3270", "current field %d FA %02X", i, FA);
        if (!FA_PROTECTED(FA)) {
          ToAttr(BA, 0);
          debug(DEBUG_TRACE, "3270", "store key at buffer %d", BA);
          ToBuffer(ascii2ebcdic(c));
          EmitChar3270(c, FA);
          CursorTN3270(1);
          ToAttr(i, FA_SETMDT(FA) | 0x80);
          debug(DEBUG_TRACE, "3270", "modify field @ %d", i);
        } else {
          debug(DEBUG_TRACE, "3270", "field is protected");
        }
      }
  }
...
Err SendAID(UInt16 c) {
  AID = c;
  debug(DEBUG_TRACE, "3270", "> AID %02X", AID);
  switch (AID) {
    case AID_PA1:
    case AID_PA2:
    case AID_PA3:
...
Err SendSpecial(UInt16 c) {
  switch (c) {
    ...
    case keyHOME:
      ...
      debug(DEBUG_TRACE, "3270", "keyHOME BA %d", BA);
      CursorTN3270(0);
      SetLinear(BA);
      break;
  }
...
static Int16 FirstField(void) {
  ...
  for (i = 0; i < BufferSize; i++) {
    if (Attr[i] && !FA_PROTECTED(Attr[i])) {
      debug(DEBUG_TRACE, "3270", "FirstField %d", i);
      return i;
    }
  }
...
static void StartField(void) {
  ...
  if (ifields < 2*FieldsSize) {
    debug(DEBUG_TRACE, "3270", "StartField %d @ %d", ifields, BA);
    b = BA >> 8;
...
static void EndField(void) {
  ...
  if (ifields % 2) {
    ...
    debug(DEBUG_TRACE, "3270", "EndField %d len %d", ifields, len);
    b = len >> 8;
    DmWrite(Fields, 2*ifields, &b, 1);
    b = len & 0x00ff;
    DmWrite(Fields, 2*ifields+1, &b, 1);
