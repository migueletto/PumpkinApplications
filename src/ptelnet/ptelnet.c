...
int PtelnetNewGlobals(void) {
  PtelnetGlobalsType *p;
  int r = -1;

  if ((p = sys_calloc(1, sizeof(PtelnetGlobalsType))) != NULL) {
    pumpkin_set_data(p);
    r = 0;
  }
...
static Err StartApplication(void) {
  ...
  if (SysLibFind(msgSerialLibrary, &p->AppSerRefnum)) {
    return -1;
  }

  if (SysLibFind(msgNetworkLibrary, &p->AppNetRefnum)) {
    return -1;
  }
...
static Boolean MainFormHandleEvent(EventPtr event) {
  ...
  switch (event->eType) {
    ...
    case frmOpenEvent:
      ...
      CtlDrawControl((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, onlineCtl)));
      CtlDrawControl((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, controlCtl)));
      CtlDrawControl((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, escCtl)));
      CtlDrawControl((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, kbdCtl)));
      CtlDrawControl((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, padCtl)));
      CtlDrawControl((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, macrosCtl)));
      ...
      CtlSetLabel((ControlPtr)FrmGetObjectPtr(frm, FrmGetObjectIndex(frm, macrosCtl)), "Macros");
  ...
  switch (event->eType) {
    case frmOpenEvent:
      ...
      switch (prefs_aux.Font) {
        case 0: FrmSetControlValue(frm, FrmGetObjectIndex(frm, sel4x6), 1); break;
        case 1: FrmSetControlValue(frm, FrmGetObjectIndex(frm, sel5x9), 1); break;
        case 2: FrmSetControlValue(frm, FrmGetObjectIndex(frm, sel8x14), 1); break;
        case 3: FrmSetControlValue(frm, FrmGetObjectIndex(frm, sel12x22), 1); break;
      }

      switch (prefs_aux.density) {
        case kDensityLow:       FrmSetControlValue(frm, FrmGetObjectIndex(frm, lowSel), 1); break;
        case kDensityDouble:    FrmSetControlValue(frm, FrmGetObjectIndex(frm, doubleSel), 1); break;
        case kDensityQuadruple: FrmSetControlValue(frm, FrmGetObjectIndex(frm, quadSel), 1); break;
      }
      if (p->density < kDensityQuadruple) {
        FrmHideObject(frm, FrmGetObjectIndex(frm, quadSel));
      }
      if (p->density < kDensityDouble) {
        FrmHideObject(frm, FrmGetObjectIndex(frm, doubleSel));
        FrmHideObject(frm, FrmGetObjectIndex(frm, lowSel));
      } else {
        FrmHideObject(frm, FrmGetObjectIndex(frm, fixedLow));
      }
    ...
    case ctlSelectEvent:
      switch (id = event->data.ctlSelect.controlID) {
        ...
        case sel12x22:
          ...
          break;
        case lowSel:
          prefs_aux.density = kDensityLow;
          handled = true;
          break;
        case doubleSel:
          prefs_aux.density = kDensityDouble;
          handled = true;
          break;
        case quadSel:
          prefs_aux.density = kDensityQuadruple;
          handled = true;
          break;
        case serialSel:
        ...
        case okBtn:
          if (prefs_aux.Font != prefs.Font || prefs_aux.density != prefs.density) {
            SetFont(prefs_aux.Font, prefs_aux.density);
            p->fontChanged = true;
          }
          InitVT100(GetRows(), GetCols());
...
static Err PhysOnline(void) {
  ...
  switch (prefs.physMode) {
    case serial:
      err = SerialOnline(&p->AppSerRefnum, prefs.Baud, prefs.Bits, prefs.Parity, prefs.StopBits, prefs.XonXoff, prefs.RtsCts, prefs.serialPort);
      if (err == 0) {
        InitVT100(GetRows(), GetCols());
      }
      break;
