...
void SendTelnet(UChar *buf, Int n, Short done)
{
  ...
  debug(DEBUG_INFO, "TELNET", "SendTelnet %d byte(s)", n);
  debug_bytes(DEBUG_INFO, "TELNET", buf, n);
  ...
  for (i = 0, k = 0; i < n; i++) {
    if (buf[i] == TELNET_IAC) {
      if ((i-k) > 0) {
        debug(DEBUG_INFO, "TELNET", "SendTelnet IAC found, sending previous %d byte(s)", i-k);
        debug_bytes(DEBUG_INFO, "TELNET", &buf[k], i-k);
        PhysSend(&buf[k], i-k, &err);
      }
      debug(DEBUG_INFO, "TELNET", "SendTelnet sending IAC IAC");
      outbuf[1] = TELNET_IAC;
  ...
  if (i > k) {
    debug(DEBUG_INFO, "TELNET", "SendTelnet last %d byte(s)", i-k);
    debug_bytes(DEBUG_INFO, "TELNET", &buf[k], i-k);
    PhysSend(&buf[k], i-k, &err);
  }

  if (telnetEor && done) {
    debug(DEBUG_INFO, "TELNET", "SendTelnet done, sending IAC EOR");
    outbuf[1] = TELNET_EOR;
    PhysSend(outbuf, 2, &err);
  }
}
