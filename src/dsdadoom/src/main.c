...
uint16_t DG_color16(uint8_t r, uint8_t g, uint8_t b) {
  return surface_color_rgb(SURFACE_ENCODING_RGB565, NULL, 0, r, g, b, 0xFF);
}
...
static uint8_t convertToDoomKey(int key) {
  ...
  switch (key) {
    case '[': key = KEYD_DEL; break;
    case ']': key = KEYD_PAGEDOWN; break;
  }
...
static void process_normal_keys(uint32_t oldMask, uint32_t newMask) {
  ...
  if (diff) {
    if (diff & keyBitPageDown) {
      addKeyToQueue(newMask & keyBitPageDown ? 1 : 0, KEYD_DOWNARROW);
    }
    if (diff & keyBitPageUp) {
      addKeyToQueue(newMask & keyBitPageUp ? 1 : 0, KEYD_UPARROW);
    }
    if (diff & keyBitLeft) {
      addKeyToQueue(newMask & keyBitLeft ? 1 : 0, KEYD_LEFTARROW);
    }
    if (diff & keyBitRight) {
      addKeyToQueue(newMask & keyBitRight ? 1 : 0, KEYD_RIGHTARROW);
    }
  }
}
...
static void process_mod_keys(uint32_t oldMask, uint32_t newMask) {
  ...
  if (diff) {
    if (diff & WINDOW_MOD_CTRL) {
      addKeyToQueue(newMask & WINDOW_MOD_CTRL ? 1 : 0, KEYD_RCTRL);
    }
    if (diff & WINDOW_MOD_SHIFT) {
      addKeyToQueue(newMask & WINDOW_MOD_SHIFT ? 1 : 0, KEYD_RSHIFT);
    }
  }
}
...
int DG_seek(dg_file_t *f, int offset, dg_seek_t whence) {
  FileOrigin origin;
  int r = -1;

  if (f) {
    switch (whence) {
      case DG_SEEK_SET: origin = vfsOriginBeginning; break;
      case DG_SEEK_CUR: origin = vfsOriginCurrent; break;
      case DG_SEEK_END: origin = vfsOriginEnd; break;
    }
    if (VFSFileSeek(f->fr, origin, offset) == errNone) {
      r = 0;
    }
  }
...
int DG_curdir(char *name, int len) {
  return VFSCurrentDir(volref, name, len) == errNone ? 0 : -1;
}
