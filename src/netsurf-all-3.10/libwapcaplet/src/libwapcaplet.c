...
#define LWC_ALLOC(s) sys_malloc(s)
#define LWC_FREE(p) sys_free(p)
...
lwc__initialise(void)
{
	...
	sys_memset(ctx, 0, sizeof(lwc_context));
	...
	sys_memset(ctx->buckets, 0, sizeof(lwc_string *) * ctx->bucketcount);
		  ...
		  lwc_string **ret)
{
	return lwc__intern(s, slen, ret,
			   lwc__calculate_hash,
			   sys_strncmp, (lwc_memcpy)sys_memcpy);
}
...
#ifndef NDEBUG
	sys_memset(str, 0xA5, sizeof(*str) + str->len);
#endif
