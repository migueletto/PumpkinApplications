...
static void nslog__normalise_category(nslog_category_t *cat)
{
	if (cat->name != NULL)
		return;
	if (cat->parent == NULL) {
		cat->name = sys_strdup(cat->cat_name);
		cat->namelen = sys_strlen(cat->name);
	} else {
		nslog__normalise_category(cat->parent);
		int bufsz = sys_strlen(cat->parent->name) + sys_strlen(cat->cat_name) + 2 /* a slash and a NUL */;
		cat->name = sys_malloc(bufsz);
		sys_snprintf(cat->name, bufsz, "%s/%s", cat->parent->name, cat->cat_name);
		cat->namelen = bufsz - 1;
	}
			      ...
			      va_list args)
{
	...
	struct nslog_cork_chain *newcork = sys_calloc(sizeof(struct nslog_cork_chain) + measured_len + 1, 1);
	if (newcork == NULL) {
		/* Wow, something went wrong */
		return;
	}
	newcork->context = *ctx;
	sys_vsnprintf(newcork->message, measured_len + 1, fmt, args);
		...
		...)
{
	...
	if (nslog__corked) {
		va_list ap2;
		va_copy(ap2, ap);
		int slen = sys_vsnprintf(NULL, 0, pattern, ap);
		va_end(ap);
