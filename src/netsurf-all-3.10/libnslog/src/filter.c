...
{
	nslog_filter_t *ret = sys_calloc(sizeof(*ret), 1);
	if (ret == NULL)
		return NSLOG_NO_MEMORY;
	ret->kind = NSLFK_CATEGORY;
	ret->refcount = 1;
	ret->params.str.ptr = sys_strdup(catname);
	ret->params.str.len = sys_strlen(catname);
				      ...
				      nslog_filter_t **filter)
{
	...
	ret->refcount = 1;
	ret->params.str.ptr = sys_strdup(filename);
	ret->params.str.len = sys_strlen(filename);
	if (ret->params.str.ptr == NULL) {
		sys_free(ret);
		return NSLOG_NO_MEMORY;
	}
				     ...
				     nslog_filter_t **filter)
{
	...
	ret->refcount = 1;
	ret->params.str.ptr = sys_strdup(dirname);
	ret->params.str.len = sys_strlen(dirname);
	if (ret->params.str.ptr == NULL) {
		sys_free(ret);
		return NSLOG_NO_MEMORY;
	}
				      ...
				      nslog_filter_t **filter)
{
	...
	ret->refcount = 1;
	ret->params.str.ptr = sys_strdup(funcname);
	ret->params.str.len = sys_strlen(funcname);
	if (ret->params.str.ptr == NULL) {
		sys_free(ret);
		return NSLOG_NO_MEMORY;
	}
...
nslog_filter_t *nslog_filter_unref(nslog_filter_t *filter)
{
	if (filter != NULL && filter->refcount-- == 1) {
		...
		case NSLFK_DIRNAME:
		case NSLFK_FUNCNAME:
			sys_free(filter->params.str.ptr);
			break;
		case NSLFK_AND:
		case NSLFK_OR:
		case NSLFK_XOR:
				   ...
				   nslog_filter_t *filter)
{
	...
	case NSLFK_CATEGORY:
		...
		if (ctx->category->name[filter->params.str.len] != '\0' &&
			...
			return false;
		return (sys_strncmp(filter->params.str.ptr,
				ctx->category->name,
	...
	case NSLFK_DIRNAME:
		...
		if ((ctx->filename[filter->params.str.len] == '/')
		    && (sys_strncmp(filter->params.str.ptr,
				ctx->filename,
				filter->params.str.len) == 0))
			return true;
		return false;
...
char *nslog_filter_sprintf(nslog_filter_t *filter)
{
	...
	switch (filter->kind) {
	case NSLFK_CATEGORY:
		ret = sys_calloc(filter->params.str.len + 5, 1);
		sprintf(ret, "cat:%s", filter->params.str.ptr);
		break;
	case NSLFK_LEVEL: {
		const char *lvl = nslog_level_name(filter->params.level);
		ret = sys_calloc(sys_strlen(lvl) + 5, 1);
		sprintf(ret, "lvl:%s", lvl);
		break;
	}
	case NSLFK_FILENAME:
		ret = sys_calloc(filter->params.str.len + 6, 1);
	...
	case NSLFK_XOR: {
		...
		ret = sys_calloc(sys_strlen(left) + sys_strlen(right) + 7, 1);
	...
	case NSLFK_NOT: {
		...
		ret = sys_calloc(sys_strlen(input) + 2, 1);
		sprintf(ret, "!%s", input);
		sys_free(input);
		break;
	}
