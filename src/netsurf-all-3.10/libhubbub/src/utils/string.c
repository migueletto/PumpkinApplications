 ...
 */
bool hubbub_string_match(const uint8_t *a, sys_size_t a_len,
		const uint8_t *b, sys_size_t b_len)
{
	if (a_len != b_len)
		return false;

	return sys_memcmp((const char *) a, (const char *) b, b_len) == 0;
}
 ...
 * \param b_len	Length of second string
 */
bool hubbub_string_match_ci(const uint8_t *a, sys_size_t a_len,
		const uint8_t *b, sys_size_t b_len)
{
	if (a_len != b_len)
		return false;
