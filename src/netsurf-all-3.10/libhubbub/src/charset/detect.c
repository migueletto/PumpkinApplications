...
#define PEEK(a)								\
	(pos < end - SLEN(a) && 					\
		sys_strncasecmp((const char *) pos, a, SLEN(a)) == 0)
		...
		const uint8_t *end)
{
	...
	while (hubbub_charset_get_attribute(pos, end,
		...
		if (namelen == SLEN("charset") && valuelen > 0 &&
				sys_strncasecmp((const char *) name, "charset",
					SLEN("charset")) == 0) {
			/* strip value */
			while (ISSPACE(*value)) {
				value++;
		...
		/* 5 */
				...
				sys_strncasecmp((const char *) name, "content",
					SLEN("content")) == 0) {
		...
		uint32_t valuelen)
{
	...
	if (value < end - SLEN("charset") &&
			sys_strncasecmp((const char *) value,
