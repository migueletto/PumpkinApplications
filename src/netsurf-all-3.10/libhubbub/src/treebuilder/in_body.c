		...
		const hubbub_token *token)
{
	...
	if (token->data.tag.n_attributes > 0) {
		uint32_t i;
		attrs = sys_malloc((token->data.tag.n_attributes + 1) *
		...
		for (i = 0; i < token->data.tag.n_attributes; i++) {
			...
			if (sys_strncmp((const char *) name, "action",
					attr->name.len) == 0) {
				action = attr;
			} else if (sys_strncmp((const char *) name, "prompt",
					attr->name.len) == 0) {
				prompt = attr;
			} else if (sys_strncmp((const char *) name, "name",
					attr->name.len) == 0) {
			} else {
				attrs[n_attrs++] = *attr;
			}
