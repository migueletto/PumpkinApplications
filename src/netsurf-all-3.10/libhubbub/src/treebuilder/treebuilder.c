		...
		hubbub_treebuilder **treebuilder)
{
	...
	tb = sys_malloc(sizeof(hubbub_treebuilder));
	if (tb == NULL)
		return HUBBUB_NOMEM;
	...
	sys_memset(&tb->context, 0, sizeof(hubbub_treebuilder_context));
	tb->context.mode = INITIAL;

	tb->context.element_stack = sys_malloc(
	...
	if (error != HUBBUB_OK) {
		sys_free(tb->context.element_stack);
		sys_free(tb);
		return error;
	}
...
hubbub_error hubbub_treebuilder_destroy(hubbub_treebuilder *treebuilder)
{
	...
	sys_free(treebuilder->context.element_stack);
		...
		const hubbub_string *tag_name)
{
	...
	for (i = 0; i < N_ELEMENTS(name_type_map); i++) {
		...
		if (sys_strncasecmp(name_type_map[i].name,
		...
		hubbub_ns ns, element_type type, void *node)
{
	...
	if (slot >= treebuilder->context.stack_alloc) {
		element_context *temp = sys_realloc(
				treebuilder->context.element_stack,
		...
		void **removed)
{
	...
	*removed = stack[index].node;
	...
	if (index < treebuilder->context.current_node) {
		sys_memmove(&stack[index], &stack[index + 1],
		...
		uint32_t stack_index)
{
	...
	entry = sys_malloc(sizeof(formatting_list_entry));
	if (entry == NULL)
		return HUBBUB_NOMEM;
