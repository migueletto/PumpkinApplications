...
rosprite_error rosprite_load(reader reader, void* ctx, struct rosprite_area** result)
{
	...
	struct rosprite_area* sprite_area = sys_malloc(sizeof(struct rosprite_area));
	...
	if (sprite_area->extension_size > 0) {
		sprite_area->extension_words = sys_malloc(sprite_area->extension_size);
	...
	sprite_area->sprites = sys_malloc(sizeof(struct rosprite*) * sprite_area->sprite_count); /* allocate array of pointers */
...
void rosprite_destroy_sprite_area(struct rosprite_area* sprite_area)
{
	...
	for (i = 0; i < sprite_area->sprite_count; i++) {
		...
		if (sprite->has_palette) sys_free(sprite->palette);
		sys_free(sprite->image);
		sys_free(sprite);
	}

	sys_free(sprite_area->sprites);
	if (sprite_area->extension_size > 0) sys_free(sprite_area->extension_words);
	sys_free(sprite_area);
}
...
rosprite_error rosprite_load_palette(reader reader, void* ctx, struct rosprite_palette** result)
{
	...
	struct rosprite_palette* palette = sys_malloc(sizeof(struct rosprite_palette));

	palette->palette = sys_malloc(sizeof(uint32_t) * 256); /* allocate 256 whether we need them all or not */
...
{
	sys_free(palette->palette);
	sys_free(palette);
}

rosprite_error rosprite_create_file_context(FILE* f, struct rosprite_file_context** result)
{
	struct rosprite_file_context* ctx = sys_malloc(sizeof(struct rosprite_file_context));
...
{
	struct rosprite_mem_context* ctx = sys_malloc(sizeof(struct rosprite_mem_context));
	if (!ctx) return ROSPRITE_NOMEM;
...
int rosprite_mem_reader(uint8_t* buf, size_t count, void* ctx)
{
	...
	sys_memcpy(buf, memctx->base + memctx->offset, copy_size);
...
rosprite_error rosprite_load_sprite(reader reader, void* ctx, struct rosprite** result)
{
	...
	struct rosprite* sprite = sys_malloc(sizeof(struct rosprite));
	struct rosprite_header* header = sys_malloc(sizeof(struct rosprite_header));
	...
	if (sprite->has_palette) {
		...
		sprite->palette = sys_malloc(sizeof(uint32_t) * sprite->palettesize);
	...
	image = sys_malloc(header->image_size);
	reader(image, header->image_size, ctx);

	if (sprite->has_mask) {
		mask = sys_malloc(header->mask_size);
...
static rosprite_error rosprite_get_mode(uint32_t sprite_mode_word, struct rosprite_mode* result)
{
	...
	sys_memcpy(result, &mode, sizeof(struct rosprite_mode));
...
static rosprite_error rosprite_load_high_color(uint8_t* image_in, uint8_t* mask, struct rosprite* sprite, struct rosprite_header* header)
{
	...
	sprite->image = sys_malloc(sprite->width * sprite->height * 4); /* all image data is 32bpp going out */
	...
	if (sprite->has_mask) sys_free(mask_state);
	return ROSPRITE_OK;
}
...
static rosprite_error rosprite_init_mask_state(struct rosprite* sprite, struct rosprite_header* header, uint8_t* mask, struct rosprite_mask_state** result)
{
	struct rosprite_mask_state* mask_state = sys_malloc(sizeof(struct rosprite_mask_state));
