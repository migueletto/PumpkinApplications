...
gif_initialise_frame_extensions(gif_animation *gif, const int frame)
{
        ...
        while (gif_data < gif_end && gif_data[0] == GIF_EXTENSION_INTRODUCER) {
                ...
                case GIF_EXTENSION_APPLICATION:
                        ...
                        if ((gif_data[1] == 0x0b) &&
                            (sys_strncmp((const char *) gif_data + 2,
                                     "NETSCAPE2.0", 11) == 0) &&
                            (gif_data[13] == 0x03) &&
                            (gif_data[14] == 0x01)) {
...
static gif_result gif_initialise_frame(gif_animation *gif)
{
        ...
        if ((int)gif->frame_holders <= frame) {
                ...
                temp_buf = (gif_frame *)sys_realloc(gif->frames, (frame + 1) * sizeof(gif_frame));
                if (temp_buf == NULL) {
                        return GIF_INSUFFICIENT_MEMORY;
                }
                gif->frames = temp_buf;
...
{
        sys_memset(gif, 0, sizeof(gif_animation));
...
gif_result gif_initialise(gif_animation *gif, size_t size, unsigned char *data)
{
        ...
        if (gif->buffer_position == 0) {
                 ...
                 */
                if (sys_strncmp((const char *) gif_data, "GIF", 3) != 0) {
                        return GIF_DATA_ERROR;
                }
                gif_data += 3;
                ...
                */
                gif->global_colour_table = sys_calloc(GIF_MAX_COLOURS, sizeof(unsigned int));
                gif->local_colour_table = sys_calloc(GIF_MAX_COLOURS, sizeof(unsigned int));
                ...
                */
                ...
                if ((gif->frames = (gif_frame *)sys_malloc(sizeof(gif_frame))) == NULL) {
                        gif_finalise(gif);
                        return GIF_INSUFFICIENT_MEMORY;
                }
                gif->frame_holders = 1;
...
void gif_finalise(gif_animation *gif)
{
        ...
        sys_free(gif->frames);
        gif->frames = NULL;
        sys_free(gif->local_colour_table);
        gif->local_colour_table = NULL;
        sys_free(gif->global_colour_table);
        gif->global_colour_table = NULL;
