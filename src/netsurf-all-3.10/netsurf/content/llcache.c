...
static nserror llcache_object_refetch(llcache_object *object)
{
	...
	if (object->cache.etag != NULL) {
		...
		sys_snprintf(headers[header_idx], len, "If-None-Match: %s",
				object->cache.etag);
	...
	if (object->cache.last_modified != 0) {
		...
		sys_snprintf(headers[header_idx], len, "If-Modified-Since: %s",
			   ...
			   size_t *datasize_out)
{
	...
	use = sys_snprintf(op, datasize, "%s", nsurl_access(object->url));
	if (use < 0) {
		goto operror;
	}
	...
	/* object size */
	use = sys_snprintf(op, datasize, "%" PRIsizet, object->source_len);
	if (use < 0) {
		goto operror;
	}
	use++; /* does not count the null */
	...
	/* number of headers */
	use = sys_snprintf(op, datasize, "%" PRIsizet, object->num_headers);
	if (use < 0) {
		goto operror;
	}
	use++; /* does not count the null */
	...
	use = sys_snprintf(op, datasize, "%" PRIsizet, cert_chain_depth);
	if (use < 0) {
		goto operror;
	}
	use++; /* does not count the null */
	...
	for (hloop = 0; hloop < cert_chain_depth; hloop++) {
		...
		use = sys_snprintf(op, datasize, "%d",
...
llcache_process_metadata(llcache_object *object)
{
	...
	if ((lnsize < 1) || (sys_sscanf(ln, "%" PRIsizet, &source_length) != 1)) {
		res = NSERROR_INVALID;
		goto format_error;
	}
	...
	if ((lnsize < 1) || (sys_sscanf(ln, "%" PRIsizet, &num_headers) != 1)) {
		res = NSERROR_INVALID;
		goto format_error;
	}
	...
	if ((lnsize < 1) || (sys_sscanf(ln, "%" PRIsizet, &ssl_cert_count) != 1)) {
		res = NSERROR_INVALID;
		goto format_error;
	}
	...
	for (hloop = 0; hloop < ssl_cert_count; hloop++) {
		...
		lnsize = strlen(ln);
		remaining -= lnsize + 1;
		if ((lnsize < 1) || (sys_sscanf(ln, "%d", &errcode) != 1)) {
			res = NSERROR_INVALID;
			goto format_error;
		}
...
static void llcache_persist(void *p)
{
	...
	for (idx = 0; idx < lst_count; idx++) {
		...
		NSLOG(llcache, DEBUG,
		      ...
		      written, (unsigned long)elapsed, (unsigned long)((written * 1000) / elapsed),
