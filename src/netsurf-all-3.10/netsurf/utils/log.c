...
void
nslog_log2(int level, const char *file, const char *func, int ln, const char *format, ...)
{
  sys_va_list ap;
  ...
  debugva_full(file, func, ln, level, "NetSurf", format, ap);
  sys_va_end(ap);
}
...
nserror nslog_init(void *ensure, int *pargc, char **argv)
{
#if 0
