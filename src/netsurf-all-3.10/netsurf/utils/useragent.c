...
*/
        ...
        len = sys_snprintf(NULL, 0, NETSURF_UA_FORMAT_STRING,
                       netsurf_version_major,
                       netsurf_version_minor,
                       sysname);
        ua_string = sys_malloc(len + 1);
        if (!ua_string) {
                /** \todo this needs handling better */
                return;
        }
        sys_snprintf(ua_string, len + 1,
...
free_user_agent_string(void)
{
	if (core_user_agent_string != NULL) {
		...
		sys_free((void *)core_user_agent_string);
