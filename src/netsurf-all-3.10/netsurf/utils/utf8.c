	     ...
	     size_t *result_len)
{
	...
	if (strncasecmp(last_cd.from, from, sizeof(last_cd.from)) == 0 &&
		...
		sys_snprintf(last_cd.from, sizeof(last_cd.from), "%s", from);
		sys_snprintf(last_cd.to, sizeof(last_cd.to), "%s", to);
		last_cd.cd = cd;
	}
			...
			size_t *outlen)
{
	...
	while (inlen > 0) {
		...
		esclen = sys_snprintf(escape, sizeof(escape), "&#x%06x;", ucs4);
		pescape = escape;
...
utf8_to_html(const char *string, const char *encname, size_t len, char **result)
{
	...
	if (strncasecmp(last_cd.from, "UTF-8", sizeof(last_cd.from)) == 0 &&
		...
		sys_snprintf(last_cd.from, sizeof(last_cd.from), "UTF-8");
		sys_snprintf(last_cd.to, sizeof(last_cd.to), "%s", encname);
		last_cd.cd = cd;
	}
	...
	while (off < len) {
		...
		if (string[off] == '&' || string[off] == '<' ||
			...
			esclen = sys_snprintf(escape, sizeof(escape),
					"&#x%06x;", string[off]);
			pescape = escape;
...
bool utf8_save_text(const char *utf8_text, const char *path)
{
	...
	out = sys_open(path, SYS_WRITE);
	if (out) {
		int res = sys_write(out, (uint8_t *)conv, sys_strlen(conv));
		if (res < 0) {
			NSLOG(netsurf, INFO, "Warning: writing data failed");
		}

		res = sys_write(out, (uint8_t *)"\n", 1);
		sys_close(out);
		free(conv);
		return (res != -1);
	}
