...
const char *filename_request(void)
{
	...
	sys_snprintf(filename_buffer, sizeof(filename_buffer), "%s%.2i", dir->prefix, i);
...
bool filename_claim(const char *filename)
{
	...
	sys_strncpy(dir_prefix, filename, 9);
	dir_prefix[8] = '\0';
...
bool filename_initialise(void)
{
  ...
  FileRef f;
	...
	directory = sys_strdup(TEMP_FILENAME_PREFIX);
	if (directory == NULL)
		return false;
	...
	for (start = directory; *start != '\0'; start++) {
		if (*start == '/') {
			*start = '\0';
      if (VFSFileOpen(1, directory, vfsModeRead, &f) != errNone) {
			  NSLOG(netsurf, INFO, "Creating \"%s\"", directory);
			  ret = VFSDirCreate(1, directory);
			  if (ret != 0) {
				  NSLOG(netsurf, INFO,
...
bool filename_flush_directory(const char *folder, int depth)
{
  ...
  char name[256];
	...
	if (depth > 0) {
		...
		prefix_start = folder + sys_strlen(folder) - depth * 3 + 1;
	}
  ...
  if (VFSFileOpen(1, folder, vfsModeRead, &parent) == errNone) {
  dirEntryIterator = vfsIteratorStart;
  info.nameP = name;
  info.nameBufLen = 256;
  while (VFSDirEntryEnumerate(parent, &dirEntryIterator, &info) == errNone) {
		int written;

		/* Ignore '.' and '..' */
		if (sys_strcmp(info.nameP, ".") == 0 ||
				sys_strcmp(info.nameP, "..") == 0)
			continue;

		written = sys_snprintf(child, sizeof(child), "%s/%s",
				folder, info.nameP);
		if (written == sizeof(child)) {
			child[sizeof(child) - 1] = '\0';
		}

    if (VFSGetAttributes(1, child, &attributes) != errNone) {
			NSLOG(netsurf, INFO, "Unable to stat %s", child);
			continue;
		}
		...
		if (depth < 3) {
			...
			del = !(attributes & vfsFileAttrDirectory);
		} else {
		...
		if (del == false && (info.nameP[0] >= '0') &&
				(info.nameP[0] <= '6') &&
				(info.nameP[1] >= '0') &&
				(info.nameP[1] <= '9') &&
				(info.nameP[2] == '\0')) {
			number = sys_atoi(info.nameP);
		...
		if (del == false && !(attributes & vfsFileAttrDirectory))
		    	continue;
...
static struct directory *filename_create_directory(const char *prefix)
{
  ...
  UInt32 attributes;
	...
	if (prefix == NULL) {
		...
		sys_sprintf(dir_prefix, "%.2i/%.2i/%.2i/",
				((index >> 12) & 63),
				((index >> 6) & 63),
				((index >> 0) & 63));
	...
	new_dir = sys_malloc(sizeof(struct directory));
	if (new_dir == NULL) {
		NSLOG(netsurf, INFO, "No memory for malloc()");
		return NULL;
	}

	sys_strncpy(new_dir->prefix, prefix, 9);
	new_dir->prefix[9] = '\0';
	 ...
	 * create the child. */
	if (prev_dir && sys_strncmp(prev_dir->prefix, new_dir->prefix, 6) == 0) {
		new_dir->prefix[8] = '\0';
		sys_sprintf(filename_directory, "%s/%s",
				TEMP_FILENAME_PREFIX,
				new_dir->prefix);
		new_dir->prefix[8] = '/';

    if (VFSGetAttributes(1, filename_directory, &attributes) == errNone && !(attributes & vfsFileAttrDirectory)) {
			if (!VFSDirCreate(1, filename_directory))
				return new_dir;
	...
	sys_sprintf(filename_directory, "%s/", TEMP_FILENAME_PREFIX);
