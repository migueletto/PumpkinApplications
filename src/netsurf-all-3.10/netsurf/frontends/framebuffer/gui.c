...
static nserror set_defaults(struct nsoption_s *defaults)
{
	...
	nsoption_setnull_charp(cookie_file, strdup(NETSURF_FB_RESPATH "/Cookies"));
...
static struct browser_window *bw;

void ns_action(int action, void *p) {
  nsurl *url;
  nserror error;
  char *s;
...
int ns_save_options(void) {
  char *options = NULL;
  int r = -1;

  netsurf_mkpath(&options, NULL, 2, NETSURF_FB_RESPATH, "Choices");
  if (options) {
    if (nsoption_write(options, NULL, NULL) == NSERROR_OK) {
      r = 0;
    }
    free(options);
  }
...
ns_main(int argc, char *argv[])
{
	...
	respaths = fb_init_resource_path(NETSURF_FB_RESPATH);
