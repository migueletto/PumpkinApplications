...
copy(nsfb_t *nsfb, nsfb_bbox_t *srcbox, nsfb_bbox_t *dstbox)
{
	...
	if (width == nsfb->width) {
		...
		sys_memmove(dstptr, srcptr, (width * height * nsfb->bpp) / 8);
	} else {
		if (srcy > dsty) {
			for (hloop = height; hloop > 0; hloop--) {
				sys_memmove(dstptr, srcptr, (width * nsfb->bpp) / 8);
				srcptr += nsfb->linelen;
				dstptr += nsfb->linelen;
			}
		} else {
...
path(nsfb_t *nsfb, int pathc, nsfb_plot_pathop_t *pathop, nsfb_plot_pen_t *pen)
{
	...
	curpt = pts = sys_malloc(ptc * sizeof(nsfb_point_t));
	if (curpt == NULL) {
		return false;
	}
...
bool select_plotters(nsfb_t *nsfb)
{
	...
	if (nsfb->plotter_fns != NULL) {
		sys_free(nsfb->plotter_fns);
	}

	nsfb->plotter_fns = sys_calloc(1, sizeof(nsfb_plotter_fns_t));
	if (nsfb->plotter_fns == NULL) {
		return false;
	}

	sys_memcpy(nsfb->plotter_fns, table, sizeof(nsfb_plotter_fns_t));
