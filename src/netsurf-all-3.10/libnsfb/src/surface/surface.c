                             ...
                             const char *name)
{
    ...
    debug(DEBUG_INFO, "NetSurf", "register surface %d: %d \"%s\"", surface_count, type, name);
...
{
    //fprintf(stderr, "surface_defaults\n");
    nsfb->width = 800;
    nsfb->height = 600;
...
nsfb_surface_get_rtns(enum nsfb_type_e type)
{
	 ...
	 */
	    ...
	    rtns = sys_malloc(sizeof(nsfb_surface_rtns_t));
	    if (rtns == NULL) {
		continue;
	    }
...
nsfb_type_from_name(const char *name)
{
    ...
    for (fend_loop = 0; fend_loop < surface_count; fend_loop++) {
        if (sys_strcmp(surfaces[fend_loop].name, name) == 0)
