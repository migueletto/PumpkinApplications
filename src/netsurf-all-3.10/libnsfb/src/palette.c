...
{
	*palette = sys_malloc(sizeof(struct nsfb_palette_s));
	if (*palette == NULL) {
		return false;
	}
	...
	(*palette)->dither_ctx.data = sys_malloc(width * 3 * sizeof(int));
...
void nsfb_palette_free(struct nsfb_palette_s *palette)
{
	if (palette != NULL) {
		if (palette->dither_ctx.data != NULL) {
			sys_free(palette->dither_ctx.data);
		}
		sys_free(palette);
	}
}
...
void nsfb_palette_dither_init(struct nsfb_palette_s *palette, int width)
{
	...
	sys_memset(palette->dither_ctx.data, 0, width * sizeof(int));
}
