...
static bmp_result bmp_info_header_parse(bmp_image *bmp, uint8_t *data)
{
        ...
        if (bmp->bpp < 16) {
                ...
                bmp->colour_table = (uint32_t *)sys_malloc(bmp->colours * 4);
                if (!bmp->colour_table)
...
static bmp_result ico_header_parse(ico_collection *ico, uint8_t *data)
{
        ...
        for (i = 0; i < count; i++) {
                ico_image *image;
                image = sys_calloc(1, sizeof(ico_image));
                if (!image)
                        return BMP_INSUFFICIENT_MEMORY;
...
{
        sys_memset(bmp, 0, sizeof(bmp_image));
                      ...
                      bmp_bitmap_callback_vt *bitmap_callbacks)
{
        ...
        sys_memset(ico, 0, sizeof(ico_collection));
